    ...
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-slate-200">
      <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm text-slate-600">
          {/* Skok na sekciju funkcionalnosti */}
          <a href={`#${LANDING_SECTIONS.features}`} className={`${ANIMATION.transitionClass} hover:text-blue-600`}>
            Funkcionalnosti
          </a>
          {/* Skok na cjenovnik */}
          <a href={`#${LANDING_SECTIONS.pricing}`} className={`${ANIMATION.transitionClass} hover:text-blue-600`}>
            Cijene
          </a>
          {/* Skok na industrije / ciljne grupe */}
          <a href={`#${LANDING_SECTIONS.industries}`} className={`${ANIMATION.transitionClass} hover:text-blue-600`}>
            Industrije
          </a>
          {/* Skok na tehničku integraciju (rute + API) */}
          <a href={`#${LANDING_SECTIONS.integration}`} className={`${ANIMATION.transitionClass} hover:text-blue-600`}>
            Integracija
          </a>
        </nav>

        {/* Akcije u headeru — prijava i registracija */}
        <div className="flex items-center gap-3">
          {/* Sekundarno dugme — vodi na postojeću Login stranicu */}
          <Link
            to={ROUTES.login}
            className={`px-4 py-2 text-sm font-medium text-slate-700 border ${COLORS.borderClass} rounded-lg ${ANIMATION.transitionClass} hover:bg-slate-50`}
          >
            Prijavi se
          </Link>
          ...
          <Link
            to={ROUTES.register}
            className={`px-4 py-2 text-sm font-medium text-white ${COLORS.primaryClass} rounded-lg ${COLORS.primaryHoverClass} ${ANIMATION.transitionClass}`}
          >
            Registruj se
          </Link>
        </div>
